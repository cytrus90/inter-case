...
pathSave = os.path.abspath('Data/data3IndexBased.csv')
...
for n in range(1,101):#(max(data.groupby('CASE_concept_name').size().sort_values())+1)):
    ev = "Ev %s" % (n)
    re = "Re %s" % (n)
    st = "St %s" % (n)
    di = "Di %s" % (n)
    X.loc[:,ev] = data.loc[:,ev].copy()
    X.loc[:,re] = data.loc[:,re].copy()
    X.loc[:,st] = data.loc[:,st].copy()
    X.loc[:,di] = data.loc[:,di].copy()
    
X.loc[:,'Event_Number'] = data.loc[:,'Event_Number'].copy()

# Dependant Variable:
Y = data[['UndesiredOutcome']]
...
Result.to_csv(os.path.abspath('Results/IndexBased_Results.csv'))
timeDF.to_csv(os.path.abspath('Results/IndexBased_ResultsTiming.csv'))
