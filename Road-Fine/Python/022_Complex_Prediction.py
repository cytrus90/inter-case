...
X = pd.DataFrame()
l = 12
for i in range(1,l):#(len(data.groupby('activity').size().sort_values())+1)):
    event = "Event %s" % (i)
    res = "Res %s" % (i)
    X.loc[:,event] = data.loc[:,event].copy()
    X.loc[:,res] =  data.loc[:,res].copy()
...
Result.to_csv(os.path.abspath('Results/Complex_Results.csv'))
timeDF.to_csv(os.path.abspath('Results/Complex_ResultsTiming.csv'))
