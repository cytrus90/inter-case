...
X = pd.DataFrame()
for i in range(1,(len(data.groupby('activity').size().sort_values())+1)):
    event = "Event %s" % (i)
    X.loc[:,event] = data.loc[:,event].copy()
X.loc[:,'resFac'] = data.loc[:,'resFac'].copy()
X.loc[:,'Event_Number'] = data.loc[:,'Event_Number'].copy()

# Dependant Variable:
Y = data[['IsLastPayment']]
...
Result.to_csv(os.path.abspath('Results/LatestPayload_Results.csv'))
timeDF.to_csv(os.path.abspath('Results/LatestPayload_ResultsTiming.csv'))
